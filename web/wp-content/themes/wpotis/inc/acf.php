 ...
 */
function wpotis_display_block_options( $args = array() ) {
	...
	// Get the block ID.
	$block_id = wpotis_get_block_id( $args['block'] );
	...
	$has_show_overlay = wpotis_has_array_key( 'show_overlay', $background_options ) && true === $background_options['show_overlay'] ? ' has-overlay' : '';
	...
	if ( $args['background_type'] ) {
		...
		if ( 'video' === $args['background_type'] ) {
			...
			$background_alt = $background_title ? sprintf( esc_attr( 'Video Background of %s', 'wpotis' ), esc_attr( $background_options['background_video_title'] ) ) : __( 'Video Background', 'wpotis' );
			...
			?>
				...
				<button class="video-toggle"><span class="screen-reader-text"><?php esc_html_e( 'Toggle video playback', 'wpotis' ); ?></span></button>
			<?php
 ...
 * @return bool
 */
function wpotis_has_block_expired( $args = array() ) {
 ...
 */
function wpotis_acf_flexible_content_layout_title( $block_title, $field, $layout, $i ) {
	...
	if ( 'none' !== $background ) {
		...
		$type = wpotis_return_flexible_content_layout_value( $background_type );
		...
		if ( 'video' === $background_type ) {
			$block_heading .= '<div style="font-size: 30px; height: 26px; width: 30px;" class="dashicons dashicons-format-video acf-flexible-title-image"><span class="screen-reader-text">' . esc_html__( 'Video', 'wpotis' ) . '</span></div>';
		}
	}
	...
	if ( wpotis_has_block_expired(
		...
		$expired .= '<span style="color: red;">&nbsp;(' . esc_html__( 'expired', 'wpotis' ) . ')</span>';
	}
...
add_filter( 'acf/fields/flexible_content/layout_title/name=content_blocks', 'wpotis_acf_flexible_content_layout_title', 10, 4 );
 ...
 */
function wpotis_return_flexible_content_layout_value( $type ) {
...
if ( function_exists( 'wpotis_acf_flexible_content_layout_title' ) ) {

	/**
	 * Set Admin Styles for Flexible Content Layout Image/Title in wpotis_acf_flexible_content_layout_title().
	 *
	 * @author WDS
	 */
	function wpotis_flexible_content_layout_title_acf_admin_head() {
	?>
	<style type="text/css">
	...
	add_action( 'acf/input/admin_head', 'wpotis_flexible_content_layout_title_acf_admin_head' );
}

/**
 * Load colors dynamically into select field from wpotis_get_theme_colors()
 *
 * @author WDS
 ...
 */
function wpotis_acf_load_color_picker_field_choices( $field ) {

	// Reset choices.
	$field['choices'] = array();

	// Grab our colors array.
	$colors = wpotis_get_theme_colors();
...
add_filter( 'acf/load_field/name=color_picker', 'wpotis_acf_load_color_picker_field_choices' );
 ...
 */
function wpotis_get_theme_colors() {
	return array(
		esc_html__( 'Alto', 'wpotis' )           => '#ddd',
		esc_html__( 'Black', 'wpotis' )          => '#000',
		esc_html__( 'Blue', 'wpotis' )           => '#21759b',
		esc_html__( 'Cod Gray', 'wpotis' )       => '#111',
		esc_html__( 'Dove Gray', 'wpotis' )      => '#666',
		esc_html__( 'Gallery', 'wpotis' )        => '#eee',
		esc_html__( 'Gray', 'wpotis' )           => '#808080',
		esc_html__( 'Gray Alt', 'wpotis' )       => '#929292',
		esc_html__( 'Light Yellow', 'wpotis' )   => '#fff9c0',
		esc_html__( 'Mineshaft', 'wpotis' )      => '#333',
		esc_html__( 'Silver', 'wpotis' )         => '#ccc',
		esc_html__( 'Silver Chalice', 'wpotis' ) => '#aaa',
		esc_html__( 'White', 'wpotis' )          => '#fff',
		esc_html__( 'Whitesmoke', 'wpotis' )     => '#f1f1f1',
	);
}
 ...
 * @return void
 */
function wpotis_display_hero_heading( $block_title ) {
 ...
 * @since NEXT
 */
function wpotis_display_link( $args = array() ) {
	echo wpotis_get_link( $args ); // WPCS: XSS Ok.
}
 ...
 */
function wpotis_get_link( $args = array() ) {
	...
	$title = wpotis_has_array_key( 'title', $button_array ) ? $button_array['title'] : esc_html__( 'Read More', 'wpotis' );

	// Get url.
	$url = wpotis_has_array_key( 'url', $button_array ) ? $button_array['url'] : '';

	// Get target, else default internal.
	$target = wpotis_has_array_key( 'target', $button_array ) ? $button_array['target'] : '_self';
	?>
