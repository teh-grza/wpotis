 ...
 * @return array
 */
function wpotis_body_classes( $classes ) {
...
add_filter( 'body_class', 'wpotis_body_classes' );

/**
 * Flush out the transients used in wpotis_categorized_blog.
 *
 * @author WDS
 * @return string
 */
function wpotis_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return false;
	}
	// Like, beat it. Dig?
	delete_transient( 'wpotis_categories' );
}
add_action( 'delete_category', 'wpotis_category_transient_flusher' );
add_action( 'save_post', 'wpotis_category_transient_flusher' );
 ...
 * @return string
 */
function wpotis_content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Read More', 'wpotis' ) . '...</a>';
}
add_filter( 'the_content_more_link', 'wpotis_content_more_link' );
 ...
 * @return string
 */
function wpotis_excerpt_more( $more ) {
	return sprintf( ' <a class="more-link" href="%1$s">%2$s</a>', get_permalink( get_the_ID() ), esc_html__( 'Read more...', 'wpotis' ) );
}
add_filter( 'excerpt_more', 'wpotis_excerpt_more' );
 ...
 * @return array
 */
function wpotis_custom_mime_types( $mimes ) {
	$mimes['svg']  = 'image/svg+xml';
	$mimes['svgz'] = 'image/svg+xml';
	return $mimes;
}
add_filter( 'upload_mimes', 'wpotis_custom_mime_types' );
...
add_shortcode( 'svg', 'wpotis_display_svg' );
 ...
 * @return string
 */
function wpotis_display_customizer_header_scripts() {

	// Check for header scripts.
	$scripts = get_theme_mod( 'wpotis_header_scripts' );
	...
	echo wpotis_get_the_content( $scripts ); // WPCS XSS OK.
}
add_action( 'wp_head', 'wpotis_display_customizer_header_scripts', 999 );
 ...
 * @return string
 */
function wpotis_display_customizer_footer_scripts() {

	// Check for footer scripts.
	$scripts = get_theme_mod( 'wpotis_footer_scripts' );
...
add_action( 'wp_footer', 'wpotis_display_customizer_footer_scripts', 999 );
...
function wpotis_add_og_tags() {
	...
	$default_base_description = ( get_bloginfo( 'description' ) ) ? get_bloginfo( 'description' ) : esc_html__( 'Visit our website to learn more.', 'wpotis' );
	...
	if ( is_category() || is_tag() || is_tax() ) {
		...
		$specify        = ( is_category() ) ? esc_html__( 'categorized in', 'wpotis' ) : esc_html__( 'tagged with', 'wpotis' );
		$queried_object = get_queried_object();
		$card_url       = get_term_link( $queried_object );
		$card_type      = 'website';

		// Translators: get the term name.
		$card_long_description = $card_description = sprintf( esc_html__( 'Posts %1$s %2$s.', 'wpotis' ), $specify, $term_name );
	}
	...
	if ( is_search() ) {
		...
		$card_long_description = $card_description = sprintf( esc_html__( 'Search results for %s.', 'wpotis' ), $search_term );
	}
...
add_action( 'wp_head', 'wpotis_add_og_tags' );
 ...
 * @author Corey Collins
 */
function wpotis_remove_archive_title_prefix( $block_title ) {
...
add_filter( 'get_the_archive_title', 'wpotis_remove_archive_title_prefix' );
 ...
 * @author Corey Collins
 */
function wpotis_remove_empty_p_tags_from_content( $content ) {
...
add_filter( 'the_content', 'wpotis_remove_empty_p_tags_from_content' );
 ...
 * @author Corey Collins
 */
function wpotis_add_chrome_lazy_loading_to_images( $attr ) {

	$attr['loading'] = 'lazy';

	return $attr;
}
add_filter( 'wp_get_attachment_image_attributes', 'wpotis_add_chrome_lazy_loading_to_images' );
 ...
 * @author Corey Collins
 */
function wpotis_allow_img_attributes( $allowedposttags ) {
...
add_filter( 'wp_kses_allowed_html', 'wpotis_allow_img_attributes', 1 );
 ...
 * @author Corey Collins
 */
function wpotis_add_lazy_loading_to_image_block( $block_content, $block ) {
...
add_filter( 'render_block', 'wpotis_add_lazy_loading_to_image_block', 10, 2 );
