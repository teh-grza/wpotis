 ...
 */
function wpotis_categorized_blog() {

	$category_count = get_transient( 'wpotis_categories' );
	...
	if ( false === $category_count ) {
		...
		set_transient( 'wpotis_categories', $category_count );
	}
 ...
 */
function wpotis_get_attachment_id_from_url( $attachment_url = '' ) {
 ...
 * @return string
 */
function wpotis_get_placeholder_unsplash( $args = array() ) {
	...
	if ( ! is_numeric( $args['width'] ) ) {
		throw new Exception( esc_html__( 'Width must be an integer', 'wpotis' ) );
	}

	if ( ! is_numeric( $args['height'] ) ) {
		throw new Exception( esc_html__( 'Height must be an integer', 'wpotis' ) );
	}
	...
	$alt = apply_filters( 'wpotis_placeholder_image_alt', esc_html__( 'WebDevStudios Placeholder Image', 'wpotis' ) );
 ...
 * @return string
 */
function wpotis_copyright_year( $atts ) {
...
add_shortcode( 'wpotis_copyright_year', 'wpotis_copyright_year', 15 );
...
if ( defined( 'WPSEO_VERSION' ) ) {
	 ...
	 * @since  NEXT
	 */
	function wpotis_move_yoast_to_bottom() {
		return 'low';
	}
	add_filter( 'wpseo_metabox_prio', 'wpotis_move_yoast_to_bottom' );
}
 ...
 * @author jomurgel
 */
function wpotis_get_the_content( $content ) {
...
add_filter( 'the_content', 'wpotis_get_the_content', 20 );
 ...
 */
function wpotis_get_theme_colors_gutenberg() {
